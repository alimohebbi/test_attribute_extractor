...
if __name__ == '__main__':
    ...
    describe_f1_per_migration(atm_df, 'migration_f1_range/table/atm.csv')
    creat_box_plots(atm_df, 'f1_score', 'migration_f1_range/plots/atm_f1_range.pdf')

    path = '../data/output/evaluation/craftdroid/oracles_included/'
    all_results_df = concat_config_results(path)
    describe_f1_per_migration(all_results_df, 'migration_f1_range/table/craft_all.csv')
    creat_box_plots(all_results_df, 'f1_score', 'migration_f1_range/plots/craft_all_f1_range.pdf')

    craft_craft_df = all_results_df[~all_results_df['src_app'].str.contains('a6|a7|a8')]
    describe_f1_per_migration(craft_craft_df, 'migration_f1_range/table/craft_craft.csv')
    creat_box_plots(craft_craft_df, 'f1_score', 'migration_f1_range/plots/craft_craft_f1_range.pdf')

    craft_atm_df = all_results_df[all_results_df['src_app'].str.contains('a6|a7|a8')]
    describe_f1_per_migration(craft_atm_df, 'migration_f1_range/table/craft_atm.csv')
    creat_box_plots(craft_atm_df, 'f1_score', 'migration_f1_range/plots/craft_atm_f1_range.pdf')

    side_by_side_df = pd.concat([atm_df, craft_atm_df])
    creat_box_plots_sbs(side_by_side_df, 'f1_score', 'migration_f1_range/plots/craft_sbs_f1_range.pdf')
